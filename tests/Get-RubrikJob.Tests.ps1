...
# Begin Pester tests
Describe -Name 'Get-RubrikJob Tests' -Fixture {
    ...
    if ($resources.$api.SuccessMock) 
    {
      Context -Name "set to API $api" -Fixture {
        It -name 'Get Job ID' -test {
          ...
          # Act
          $value1 = (ConvertFrom-Json -InputObject $resources.$api.SuccessMock).id
          (Get-RubrikJob -id $value1 -api $api).id | Should BeExactly $value1
