...
Describe -Name 'Public/Get-RubrikReplicationSource' -Tag 'Public', 'Get-RubrikReplicationSource' -Fixture {
    #region init
    ...
    Context -Name 'Returned Results' {
        ...
        It -Name 'No parameters returns all results' -Test {
            @( Get-RubrikReplicationSource).Count |
                Should -BeExactly 3
        }
        It -Name 'Server should be DataLocation:::22222-22222-33333' -Test {
            @( Get-RubrikReplicationSource -Name 'Cluster_2').id |
    ...
    Context -Name 'Parameter Validation' {
        It -Name 'ParameterSet Validation' -Test {
            { Get-RubrikReplicationSource -Name 'test' -id '11111' } |
                Should -Throw "Parameter set cannot be resolved using the specified named parameters."
        }
        It -Name 'ID cannot be null or empty' -Test {
            { Get-RubrikReplicationSource -Id '' } | 
                Should -Throw "Cannot validate argument on parameter 'Id'."
        }
        It -Name 'Name cannot be null or empty' -Test {
            { Get-RubrikReplicationSource -Name '' } | 
                Should -Throw "Cannot validate argument on parameter 'sourceClusterName'."
        }              
    }
}