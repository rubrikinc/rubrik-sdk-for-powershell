...
Describe -Name 'Public/Get-RubrikNutanixVM' -Tag 'Public', 'Get-RubrikNutanixVM' -Fixture {
    #region init
    ...
    Context -Name 'Parameter/SLA' {
        ...
        It -Name 'should overwrite $SLAID' -Test {
            ( Get-RubrikNutanixVM -SLA 'test-valid_sla_name' ).Name |
    ...
    Context -Name 'Parameter/SLAID' {
        ...
        It -Name 'should not overwrite $SLAID' -Test {
            ( Get-RubrikNutanixVM -SLAID 'test-valid_sla_id' ).Name |
                Should -BeExactly 'test-vm1'
        }
        Assert-VerifiableMock
        Assert-MockCalled -CommandName Test-RubrikConnection -ModuleName 'Rubrik' -Times 1
