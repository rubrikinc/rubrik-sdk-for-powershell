...
Describe -Name 'Public/Remove-RubrikHyperVMount' -Tag 'Public', 'Remove-RubrikHyperVMount' -Fixture {
    #region init
    ...
    Context -Name 'Parameter Validation' {
        ...
        Mock -CommandName Submit-Request -Verifiable -ModuleName 'Rubrik' -MockWith {
            @{
                'id'       = 'UNMOUNT_HYPERV_SNAPSHOT_11111'
                'status'   = 'QUEUED'
                'progress' = '0'
            }
        }
        ...
        It -Name 'Should remove mount with QUEUED' -Test {
            ( Remove-RubrikHyperVMount -Id 'snapshotid').status |
                Should -BeExactly 'QUEUED'
        }
        
        It -Name 'Parameter id is missing' -Test {
            { Remove-RubrikHyperVMount -Id  } |
